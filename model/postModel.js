...
function getAll() {
    return db.query('SELECT post.id, post.titulo, post.texto, post.users_id FROM post LEFT JOIN users ON users.id = post.users_id');
  }
function save(post) {
...
function getPost(id) {
    return db.query('SELECT post.id, post.titulo, post.texto, post.users_id  FROM post LEFT JOIN users ON users.id = post.users_id WHERE post.id = ?', [id]);
  }
